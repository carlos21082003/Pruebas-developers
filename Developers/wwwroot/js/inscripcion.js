...
function loadDataTable() {
    datatable = $('#table_inscripciones').DataTable({
        language: {
        ...
        ajax: {
            url: "/Inscripciones/ListarTodos"
        },
        columns: [
            { data: "inscripcionesId", width: "10%", className: "text-center", orderable: false },
            { data: "course.name" },
            {
                data: "student",
            ...
            {
                data: "sessionDateInscripciones",
                render: function (data) { return new Date(data).toLocaleDateString(); }
            },
            { data: "hoursInscripciones", className: "text-center" },
            {
                data: "status",
            ...
            {
                data: "inscripcionesId",
                render: function (data) {
                    return `
                        <a href="/Inscripciones/Edit/${data}" class="btn btn-sm btn-success text-white" style="cursor:pointer;">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <a href="/Inscripciones/Details/${data}" class="btn btn-sm btn-info text-white" style="cursor:pointer;">
                            <i class="bi bi-list-check"></i>
                        </a>
                        <a onclick=Delete("/Inscripciones/Delete/${data}") class="btn btn-sm btn-danger text-white" style="cursor:pointer;">
